...
```{r}
datos %>% select(FECHA) %>% unique()
```



```{r}
VentasMensuales = DatosDF %>% group_by(MES) %>%  dplyr::summarize(Num_Ventas =  n(),PrecioMedio = mean(PRECIO_CON_IMPUESTOS) )
...
VentasMensuales %>% kable(booktabs=TRUE,col.names = c("Mes","Volumen Ventas","Precio Medio")) %>% kable_styling(latex_options = "striped")
```

```{r}
Ventas_DiaSem = DatosDF %>% group_by(DIA_SEMANA) %>% dplyr::summarise(
  Ventas = n() , PrecioMedio = mean(PRECIO_CON_IMPUESTOS) , mean(CANTIDAD) )
...
Ventas_DiaSem %>% kable(booktabs=TRUE,col.names = c("Dia","Volumen Ventas","Precio Medio")) %>% kable_styling(latex_options = "striped")
```



```{r}
ggplot(Ventas_DiaSem, aes(fill =DIA_SEMANA , x = DIA_SEMANA)) + 
  geom_histogram()

ggplot(data=Ventas_DiaSem, aes(x=DIA_SEMANA, y=Ventas, fill=DIA_SEMANA)) + 
