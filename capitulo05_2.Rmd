...
En este paso, vamos a llevar a cabo la limpieza de éstos para su posterior estudio, representación y modelado. En este punto del proceso, trataremos de encontrar, corregir o eliminar registros erróneos en los datos. 
...
```{r}
boxplot(dataset[,c(5,9)],main="BoxPlot de las variables cantidad e importe con impuestos")
```
...
Ventas_Prod = Prod %>% mutate(VolumenTotal = Prod$CANTIDAD * Prod$n) %>% group_by(CODIGO) %>%  dplyr::summarize(VENTAS_TOTALES =  sum(VolumenTotal) )
```



```{r , fig.height=3 , fig.width=3}
ggplot(Ventas_Prod, aes(fill =CODIGO , x = CODIGO , y=VENTAS_TOTALES)) + 
...
```{r , fig.pos='H'}
dataset %>% select(ID_TICKET,CODIGO,IMPORTE_CON_IMPUESTOS) %>% group_by( CODIGO) %>% 
  dplyr::summarise(IMPORTE_MEDIO= mean(IMPORTE_CON_IMPUESTOS)) %>% 
  kable(booktabs=TRUE, caption="Importe medio de venta según producto",align = 'c')
```
...
Ventas_MES_TOT = dataset %>% group_by(ID_TICKET, MES) %>% dplyr::summarise(
  Ventas = n()   ) %>% group_by(MES) %>% dplyr::summarise(VENTAS_TOTALES = n() ) %>% select(VENTAS_TOTALES) %>% mutate(CODIGO=rep('TOTAL',6))
...
```{r}
VentasMensuales_Prod = dataset %>% group_by(ID_TICKET, MES,CODIGO) %>% dplyr::summarise(
  Ventas = n()   ) %>% group_by(MES,CODIGO) %>% dplyr::summarise(VENTAS_TOTALES = n() ) %>% select(VENTAS_TOTALES,CODIGO)
VentasMensuales_Prod$MES=c(
ifelse(VentasMensuales_Prod$MES == 1 , "Enero",
       ifelse(
          VentasMensuales_Prod$MES == 8 , "Agosto",
       ifelse(
         VentasMensuales_Prod$MES == 9 , "Septiembre",
         ifelse( VentasMensuales_Prod$MES == 10 , "Octubre",
                 ifelse(
                    VentasMensuales_Prod$MES == 11 , "Noviembre",
                    ifelse(VentasMensuales_Prod$MES == 12 , "Diciembre"," " ))))))
)


DatosMes = rbind.data.frame(VentasMensuales_Prod,VentasMensuales[,c(1,3,4)])
```


```{r }
# Aquí no ventas medias, ya que solo hay un mes, no hay varios años
ggplot(DatosMes, aes(fill =CODIGO , x = MES , y=VENTAS_TOTALES)) + 
  geom_histogram(stat="identity", position="dodge") +  
...
scale_fill_brewer(palette="Blues")+
  theme_minimal()+theme_classic()+ #theme(legend.position = 'none')+
  ggtitle("Comparación del volumen de ventas según mes del año")
```



Podemos ver una tendencia creciente del número de ventas total, donde se tiende a vender más cada mes. Sin embargo, encontramos que el mes en el que más ventas se han realizado ha sido en el mes de octubre, superando las 18 mil ventas. Además, si comparamos el volumen de ventas de cada producto, se vende más el producto 20445, sin embargo, en los últimos meses el número de artículos vendido de cada producto se igualó.
...
```{r}
NumMedio_Ventas_P1 = rep(NA,7)
DIA_SEMANA_P1=rep(1:7,by=1)
for (i in 1:length(DIA_SEMANA_P1)) {
 NumMedioVentas_P1 =  (dataset %>% group_by(DIA_SEMANA) %>% filter(CODIGO == 20445 & DIA_SEMANA==i ) %>% group_by(FECHA) %>% dplyr::summarise(n()) %>% as.data.frame())[,2] %>% mean()
NumMedio_Ventas_P1[i] =  NumMedioVentas_P1
Ventas_DiaSem_P1 = cbind.data.frame(DIA_SEMANA_P1, NumMedio_Ventas_P1)
Ventas_DiaSem_P1= Ventas_DiaSem_P1 %>% mutate(CODIGO = rep(20445,7))
}


colnames(Ventas_DiaSem_P1) = c("DIA_SEMANA","VENTAS_MEDIAS","CODIGO")

NumMedio_Ventas_P2 = rep(NA,7)
DIA_SEMANA_P2=rep(1:7,by=1)
for (i in 1:length(DIA_SEMANA_P2)) {
 NumMedioVentas_P2 =  (dataset %>% group_by(DIA_SEMANA) %>% filter(CODIGO == 22336 & DIA_SEMANA==i ) %>% group_by(FECHA) %>% dplyr::summarise(n()) %>% as.data.frame())[,2] %>% mean()
NumMedio_Ventas_P2[i] =  NumMedioVentas_P2
Ventas_DiaSem_P2 = cbind.data.frame(DIA_SEMANA_P2, NumMedio_Ventas_P2)
Ventas_DiaSem_P2= Ventas_DiaSem_P2 %>% mutate(CODIGO = rep(22336,7))
}


colnames(Ventas_DiaSem_P2) = c("DIA_SEMANA","VENTAS_MEDIAS","CODIGO")

VentasSemanales_Medias_Codigo = rbind.data.frame(Ventas_DiaSem_P1,Ventas_DiaSem_P2)


VentasSemanales_Medias_Codigo$DIA_SEMANA=c(
ifelse(VentasSemanales_Medias_Codigo$DIA_SEMANA == 1 , "Lunes",
       ifelse(
          VentasSemanales_Medias_Codigo$DIA_SEMANA == 2 , "Martes",
       ifelse(
        VentasSemanales_Medias_Codigo$DIA_SEMANA== 3 , "Miércoles",
         ifelse( VentasSemanales_Medias_Codigo$DIA_SEMANA == 4 , "Jueves",
                 ifelse(
                    VentasSemanales_Medias_Codigo$DIA_SEMANA == 5 , "Viernes",
                    ifelse(VentasSemanales_Medias_Codigo$DIA_SEMANA == 6 , "Sábado",
                           ifelse(VentasSemanales_Medias_Codigo$DIA_SEMANA == 7 , "Domingo",""))))))))
```
...
Ventas_DiaSem= Ventas_DiaSem %>% mutate(CODIGO = rep("TOTAL",7))
}
...
colnames(Ventas_DiaSem)=c("DIA_SEMANA","VENTAS_MEDIAS","CODIGO")


Ventas_MED_DIA_SEM= rbind.data.frame(VentasSemanales_Medias_Codigo,Ventas_DiaSem)
...
```{r , ventas_TOTALES }
VentasSemanales_Prod=dataset %>% group_by(ID_TICKET, DIA_SEMANA,CODIGO) %>% dplyr::summarise(
  Ventas = n()   ) %>% group_by(DIA_SEMANA,CODIGO) %>% dplyr::summarise(VENTAS_TOTALES = n() ) %>% select(VENTAS_TOTALES,CODIGO)
...
Ventas_DiaSem_TOT= Ventas_DiaSem_TOT %>% mutate(CODIGO = rep("TOTAL",7),DIA_SEMANA=1:7)
Ventas_DiaSem_TOT=Ventas_DiaSem_TOT[,c(3,1,2)]
Ventas_TOT_DIA_SEM = rbind.data.frame(VentasSemanales_Prod,Ventas_DiaSem_TOT)


Ventas_TOT_DIA_SEM$DIA_SEMANA=c(
ifelse(Ventas_TOT_DIA_SEM$DIA_SEMANA == 1 , "Lunes",
       ifelse(
          Ventas_TOT_DIA_SEM$DIA_SEMANA == 2 , "Martes",
       ifelse(
        Ventas_TOT_DIA_SEM$DIA_SEMANA== 3 , "Miércoles",
         ifelse( Ventas_TOT_DIA_SEM$DIA_SEMANA == 4 , "Jueves",
                 ifelse(
                    Ventas_TOT_DIA_SEM$DIA_SEMANA == 5 , "Viernes",
                    ifelse(Ventas_TOT_DIA_SEM$DIA_SEMANA == 6 , "Sábado",
                           ifelse(Ventas_TOT_DIA_SEM$DIA_SEMANA == 7 , "Domingo",""))))))))
```

```{r}
VENTAS_DIA_SEMANA = merge(Ventas_MED_DIA_SEM,Ventas_TOT_DIA_SEM,by=intersect(names(Ventas_MED_DIA_SEM),names(Ventas_TOT_DIA_SEM)))
```



```{r}
ggplot(VENTAS_DIA_SEMANA, aes(fill =CODIGO , x = DIA_SEMANA , y=VENTAS_MEDIAS)) + 
  geom_histogram(stat="identity", position="dodge") +  
   scale_x_discrete(limits = unique(Ventas_MED_DIA_SEM$DIA_SEMANA) )+
labs(x="Día de la semana" , y = "Volumen medio de ventas", caption = "Fuente: Elaboración propia con datos de ventas")+
scale_fill_brewer(palette="Blues")+
  theme_minimal()+theme_classic()+ # theme(legend.position = 'none')+
...
Observando el gráfico, vemos que el Sábado es el día de la semana donde el volumen de ventas es mayor, superando una media de mil ventas. Por el contrario, el Domingo es el día donde se registra un volumen medio de ventas diario considerablemente inferior.




```{r}
# ggplot(DatosSemana, aes(fill =CODIGO , x = DIA_SEMANA , y=VENTAS_TOTALES)) + 
#   geom_histogram(stat="identity", position="dodge") +  
#    scale_x_discrete(limits = Ventas_DiaSem$DIA_SEMANA)+
# labs(x="Día de la semana" , y = "Volumen total de ventas", caption = "Fuente: Elaboración # propia con datos")+
# scale_fill_brewer(palette="Blues")+
#   theme_minimal()+theme_classic()+ # theme(legend.position = 'none')+
#   ggtitle("Volumen total de vnetas según día de la semana")
```
...
Si estudiamos la variable descuento, vemos que hay un descuento hasta en `r dataset[dataset$DESCUENTO !=0,] %>% nrow()` ventas, siendo `r round(dataset[dataset$DESCUENTO !=0,'DESCUENTO'] %>% mean(),2)` € el descuento medio aplicado.
...
```{r}
Descuento = dataset[dataset$DESCUENTO !=0,] %>% group_by(CODIGO)  %>%   dplyr::summarise(n())
```



```{r}
ConDescuento = dataset[dataset$DESCUENTO !=0,] %>% group_by( ID_TICKET) %>%  dplyr::summarise( Importe_Imp= sum(IMPORTE_CON_IMPUESTOS)   )
SinDescuento = dataset[dataset$DESCUENTO ==0,] %>% group_by( ID_TICKET) %>%  dplyr::summarise( Importe_Imp=
                                                                                                 sum(IMPORTE_CON_IMPUESTOS)    )


FechasD = dataset[dataset$DESCUENTO !=0,] %>% select(FECHA) %>% unique()
FechasND=dataset[dataset$DESCUENTO ==0,] %>% select(FECHA) %>% unique()
```
